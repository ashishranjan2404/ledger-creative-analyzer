...
- `FINNHUB_KEY` (**optional**) — enables L5 `current` valuation multiples (P/E / EV/EBITDA / FCF yield) by fetching the live quote (`/quote.c`) and shares outstanding (`/stock/profile2.shareOutstanding`, MILLIONS) per ticker. When unset OR on per-ticker fetch failure, `fetchValuationContext` receives `NaN/NaN` and the renderer prints `n/a` for `current` columns; `5yr median` + sector context still populate from XBRL history. No retries, no caching (Loop 11).
...
- Price feed + shares-outstanding feed populate when `FINNHUB_KEY` is set (Finnhub free tier: `/quote` + `/stock/profile2`); the key is **optional** — without it, `runDeepDive()` passes `NaN` to `fetchValuationContext` and `current` multiples render as `n/a` (`5yr median` + sector still ship). Per-ticker fetch failures degrade to the same NaN fallback rather than aborting the card.
