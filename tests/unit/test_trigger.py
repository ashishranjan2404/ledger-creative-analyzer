...
def test_detects_valid_trigger_variants(text):
    det = TriggerDetector()
    assert det.check(text, wall_clock_ts=1.0) is True
...
def test_rejects_non_triggers(text):
    det = TriggerDetector()
    assert det.check(text, wall_clock_ts=1.0) is False
...
def test_cooldown_suppresses_refire():
    ...
    with patch("spec_lesson.trigger.detector.time") as mock_time:
        ...
        assert det.check("ok claude build that", wall_clock_ts=100.0) is True
        # within cooldown (wall=120, only 20s elapsed)
        mock_time.monotonic.return_value = 120.0
        assert det.check("ok claude build this", wall_clock_ts=120.0) is False
        # after cooldown (wall=131, 31s elapsed > 30s)
        mock_time.monotonic.return_value = 131.0
        assert det.check("ok claude build it", wall_clock_ts=131.0) is True
...
def test_cooldown_uses_wall_clock_not_audio_timestamp():
    ...
    with patch("spec_lesson.trigger.detector.time") as mock_time:
        mock_time.monotonic.return_value = 100.0
        fired = det.check("ok claude build that", wall_clock_ts=1000.0)
    ...
    with patch("spec_lesson.trigger.detector.time") as mock_time:
        ...
        fired_after_reconnect = det.check("ok claude build this", wall_clock_ts=2.0)
    ...
    with patch("spec_lesson.trigger.detector.time") as mock_time:
        ...
        suppressed = det.check("ok claude build it", wall_clock_ts=3.0)
